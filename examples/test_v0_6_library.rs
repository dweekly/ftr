...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(ref isp) = result.isp_info {
        println!("   Detected ISP: AS{} ({})", isp.asn, isp.name);
    }
    ...
    for hop in &result.hops {
        if let Some(addr) = hop.addr {
            ...
            let rtt = hop
                .rtt_ms()
                .map_or("*".to_string(), |ms| format!("{:.3} ms", ms));
    ...
    println!(
        "   Target: {} ({})",
        result_transit.target, result_transit.target_ip
    );
