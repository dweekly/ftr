...
async fn test_destination_asn_field() {
    ...
    assert!(result.destination_asn.is_none(), 
...
fn test_segment_serialization() {
    ...
    assert_eq!(transit_json, "\"Transit\"", "Transit should serialize as \"Transit\"");
    assert_eq!(dest_json, "\"Destination\"", "Destination should serialize as \"Destination\"");
}
...
fn test_public_ip_classification() {
    ...
    for ip in &private_ips {
        assert!(ip.is_private() || ip.is_loopback() || ip.is_link_local(),
            "{} should be a private/internal IP", ip);
    }
    ...
    assert!(Ipv4Addr::new(100, 64, 0, 1) >= cgnat_start && 
            Ipv4Addr::new(100, 64, 0, 1) <= cgnat_end,
            "100.64.0.1 should be in CGNAT range");
    assert!(Ipv4Addr::new(100, 127, 255, 254) >= cgnat_start && 
...
async fn test_localhost_trace_segments() {
    ...
    if let Some(first_hop) = result.hops.first() {
        if first_hop.addr == Some(std::net::IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1))) {
            assert_eq!(first_hop.segment, SegmentType::Lan,
